    ...
    logs.each do |log|
      ...
      when 'said'
        if log.npc_chara_name == 'Game Master' #GM
          body += "<tr><td>#{log.npc_chara_name}</td><td colspan='4'>#{simple_format(log.body)}</td></tr>"
          ...
          body += "<tr><td>#{log.player.character.name}</td><td colspan='4'>#{simple_format(log.body)}</td></tr>"
        else
          body += "<tr><td>#{log.player.player_type}<br/>(#{log.npc_chara_name})</td><td colspan='4'>#{simple_format(log.body)}</td></tr>"
        end
      when 'dice'
      ...
      when 'move'
        #TODO:マップ移動処理
      when 'first'
        body += "<tr><td colspan='5'>#{simple_format(log.body)}</td></tr>"
      else
